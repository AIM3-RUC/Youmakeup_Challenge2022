...
def train(opt):
    ...
    while True:
        ...
        if (epoch % opt.save_checkpoint_every == 0) and (epoch >= opt.min_epoch_when_save):
                    #opt.save_checkpoint_every=1                  opt.min_epoch_when_save=-1
            # Save model
            ...
            eval_score, eval_loss = evaluate(model, criterion, postprocessors, val_loader, 
                                             result_json_path, logger=logger, alpha=opt.ec_alpha, 
                                             device=opt.device, debug=opt.debug)
